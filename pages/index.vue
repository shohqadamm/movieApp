   ...
   <div v-if="searchInput !== ''" id="movies-grid" class="movies-grid">
    <div v-for="(movie,index) in searchedMovies" :key="index" class="movie">
      ...
      <div class="info">
        ...
        <p class="release">
          Realsed: {{
            new Date(movie.release_date).toLocale.String('en-US',{
              month: 'long',
              day: 'numeric',
              year: 'numeric',
            })
  ...
  methods: {
    async getmovies(){
      const data = axios.get(
        'https://api.themoviedb.org/3/movie/now_playing?api_key=b8a7c2b3c1891a4f2e697f3983ed0d2e'
      )
      ...
      .movie {
        ...
        .movie-img {
          ...
          .overview {
            ...
            position: absolute;
            bottom: 0;
            background-color: rgba(219, 63, 28, 0.9);
            padding: 12px;
            color: #fff;
